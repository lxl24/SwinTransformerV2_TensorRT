...
def load_checkpoint(config, model, optimizer, lr_scheduler):
    ...
    #         config.MODEL.RESUME, map_location=torch.device('cuda:0'), check_hash=True)
    # else:
    ...
    #     optimizer.load_state_dict(checkpoint['optimizer'],map_location)
...
def load_pretrained(config, model, logger):
    ...
    checkpoint = torch.load(config.MODEL.PRETRAINED, map_location=torch.device('cuda:1'))
