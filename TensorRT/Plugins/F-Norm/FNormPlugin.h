...
namespace
{
static const char* PLUGIN_NAME{"L2Norm"};
...
// +------- Plugin body ----------------------------------------------------------------------------
class L2NormPlugin: public IPluginV2DynamicExt
{
private:    
    std::string name_;
...
public:
    L2NormPlugin(const std::string& name) : name_(name)
    {
        WHERE_AM_I();
    }

    L2NormPlugin(const std::string& name, const void* data, size_t length) : name_(name)
    {
        WHERE_AM_I();
    }
...
class L2NormPluginCreator : public IPluginCreator
{
private:
...
public:
    ...
    {
        WHERE_AM_I();
        return new L2NormPlugin(name);
    }

    IPluginV2* deserializePlugin(const char* name, const void* serialData, size_t serialLength) noexcept override
    {
        return new L2NormPlugin(name, serialData, serialLength);
    }
